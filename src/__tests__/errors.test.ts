...
describe("classifyError", () => {
  describe("authentication errors", () => {
    ...
    it("does NOT classify exit code 1 as auth when 'tool' is mentioned", () => {
      const result = classifyError(
        "Claude Code process exited with code 1 - tool error",
      );
    ...
    it("does NOT classify exit code 1 as auth when 'mcp' is mentioned", () => {
      const result = classifyError(
        "Claude Code process exited with code 1 - mcp server crashed",
      );
      expect(result.status).toBe(502);
      expect(result.type).toBe("api_error");
    });
  ...
  describe("stale session detection", () => {
    it("detects 'No message found with message.uuid' errors", () => {
      expect(
        isStaleSessionError(
          new Error(
            "No message found with message.uuid of: e663b687-6d08-4cc4-b9a9-5245ce8f1e07",
          ),
        ),
      ).toBe(true);
    });
    ...
    it("detects the error embedded in longer messages", () => {
      expect(
        isStaleSessionError(
          new Error(
            "claude code returned an error result: No message found with message.uuid of: abc123",
          ),
        ),
      ).toBe(true);
    });
    ...
    it("detects 'No conversation found with session ID' errors", () => {
      expect(
        isStaleSessionError(
          new Error(
            "No conversation found with session ID: 2e9e868c-ab59-482c-ae28-3b60ec9cb95b",
          ),
        ),
      ).toBe(true);
    });
    ...
    it("detects 'No conversation found to continue' errors", () => {
      expect(
        isStaleSessionError(new Error("No conversation found to continue")),
      ).toBe(true);
    });

    it("detects 'No conversations found to resume' errors", () => {
      expect(
        isStaleSessionError(new Error("No conversations found to resume")),
      ).toBe(true);
      expect(
        isStaleSessionError(new Error("No conversations found to resume.")),
      ).toBe(true);
    ...
    it("returns false for unrelated errors", () => {
      ...
      expect(isStaleSessionError(new Error("authentication failed"))).toBe(
        false,
      );
    });

    it("returns false for non-Error values", () => {
      expect(isStaleSessionError("No message found with message.uuid")).toBe(
        false,
      );
  ...
  describe("extra usage required", () => {
    ...
    it("detects lowercase variant", () => {
      expect(
        isExtraUsageRequiredError("extra usage is required for 1m context"),
      ).toBe(true);
    });
    ...
    it("matches 'extra usage' without '1m' suffix (broadened in ac81c3e)", () => {
      expect(isExtraUsageRequiredError("extra usage enabled")).toBe(true);
    });
