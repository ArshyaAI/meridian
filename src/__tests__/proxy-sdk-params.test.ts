...
const { createProxyServer, clearSessionCache } =
...
describe("SDK param passthrough — header overrides", () => {
  ...
  it("MERIDIAN_OPUS_EFFORT injects effort for opus models when body has none", async () => {
    process.env.MERIDIAN_OPUS_EFFORT = "max";
    try {
      const app = createTestApp();
      await post(app, { ...BASE_BODY, model: "claude-opus-4-6" });
      expect(capturedOptions.effort).toBe("max");
    } finally {
      delete process.env.MERIDIAN_OPUS_EFFORT;
    }
  });

  it("MERIDIAN_OPUS_EFFORT does NOT inject for non-opus models", async () => {
    process.env.MERIDIAN_OPUS_EFFORT = "max";
    try {
      const app = createTestApp();
      await post(app, BASE_BODY); // haiku
      expect(capturedOptions.effort).toBeUndefined();
    } finally {
      delete process.env.MERIDIAN_OPUS_EFFORT;
    }
  ...
  it("body effort takes precedence over MERIDIAN_OPUS_EFFORT", async () => {
    process.env.MERIDIAN_OPUS_EFFORT = "max";
    try {
      const app = createTestApp();
      await post(app, {
  ...
  it("x-opencode-task-budget header overrides body task_budget", async () => {
    ...
    await post(
      app,
      { ...BASE_BODY, task_budget: { total: 1000 } },
      {
  ...
  it("malformed x-opencode-thinking header is ignored — request succeeds", async () => {
    ...
    expect(
      calls.some((msg) => msg.includes("malformed x-opencode-thinking")),
    ).toBe(true);
...
describe("Usage logging", () => {
  ...
  it("logs usage line after streaming response", async () => {
    ...
    expect(
      calls.some((msg) => msg.includes("[PROXY]") && msg.includes("usage:")),
    ).toBe(true);
    logSpy.mockRestore();
  });
...
describe("GET /v1/sessions/:claudeSessionId/context-usage", () => {
  ...
  it("returns 404 for unknown session ID", async () => {
    ...
    const res = await app.fetch(
      new Request("http://localhost/v1/sessions/unknown-id/context-usage"),
    );
  ...
  it("returns usage after a completed request", async () => {
    ...
    await post(
      app,
      { ...BASE_BODY, "x-opencode-session": "agent-session-abc" },
      {
    ...
    const res = await app.fetch(
      new Request(
        `http://localhost/v1/sessions/${claudeSessionId}/context-usage`,
      ),
    );
    expect(res.status).toBe(200);
  ...
  it("returns 404 when session exists but has no usage data", async () => {
    ...
    const res = await app.fetch(
      new Request(
        "http://localhost/v1/sessions/sess_no_usage_001/context-usage",
      ),
    );
    expect(res.status).toBe(404);
  });
  ...
  it("uses the Claude session ID from the response, not the agent session ID", async () => {
    ...
    const byAgent = await app.fetch(
      new Request(
        `http://localhost/v1/sessions/${agentSessionId}/context-usage`,
      ),
    );
