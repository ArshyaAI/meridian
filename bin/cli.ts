...
if (args[0] === "profile") {
  const {
    ...
    profileLogin,
    profileHelp,
  } = await import("../src/proxy/profileCli");
...
if (args[0] === "setup") {
  ...
  if (result.alreadyConfigured) {
    ...
    if (result.removedStale.length > 0) {
      console.log(
        `  Removed ${result.removedStale.length} stale plugin entr${result.removedStale.length === 1 ? "y" : "ies"}`,
      );
    }
...
process.on("unhandledRejection", (reason) => {
  console.error(
    `[PROXY] Unhandled rejection (recovered): ${reason instanceof Error ? reason.message : reason}`,
  );
});

const port = parseInt(
  process.env.MERIDIAN_PORT ?? process.env.CLAUDE_PROXY_PORT ?? "3456",
  10,
);
const host =
  process.env.MERIDIAN_HOST ?? process.env.CLAUDE_PROXY_HOST ?? "127.0.0.1";
const idleTimeoutSeconds = parseInt(
  process.env.MERIDIAN_IDLE_TIMEOUT_SECONDS ??
    process.env.CLAUDE_PROXY_IDLE_TIMEOUT_SECONDS ??
    "120",
  10,
);
const roundRobin =
  process.env.MERIDIAN_ROUND_ROBIN === "1" ||
  process.env.MERIDIAN_ROUND_ROBIN === "true";
...
try {
  ...
  console.error(
    `[meridian] Failed to parse MERIDIAN_PROFILES: ${e instanceof Error ? e.message : e}`,
  );
}
...
export async function runCli(
  ...
  try {
    const { findOpencodeConfigPath, checkPluginConfigured, findPluginPath } =
    ...
    if (existsSync(configPath) && !checkPluginConfigured(configPath)) {
      const pluginPath = findPluginPath(import.meta.url);
      console.error(
        "\x1b[33m⚠ Meridian plugin not found in OpenCode config.\x1b[0m",
      );
      console.error(
        "  Session tracking and subagent model selection won\'t work.",
      );
  ...
  try {
    ...
    if (!auth.loggedIn) {
      console.error(
        "\x1b[31m✗ Not logged in to Claude.\x1b[0m Run: claude login",
      );
      process.exit(1);
    }
    if (auth.subscriptionType !== "max") {
      console.error(
        `\x1b[33m⚠ Claude subscription: ${auth.subscriptionType || "unknown"} (Max recommended)\x1b[0m`,
      );
    }
  } catch {
    console.error(
      "\x1b[33m⚠ Could not verify Claude auth status. If requests fail, run: claude login\x1b[0m",
    );
  }
  ...
  if (!profiles) {
    const { enableDiskProfileDiscovery } =
      await import("../src/proxy/profiles");
